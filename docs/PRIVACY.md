...
使用者主動套用、檢查、開啟自動模式或使用觀察頁鑑定建議時，插件沿用 iNaturalist 原有的 AI 建議請求。網站自行把辨識圖片及可用的位置／日期送到官方辨識服務，沿用網站登入驗證。插件只在頁面記憶體中讀取原生回應的分類 ID 與 `combined_score`，不另行呼叫辨識 API，不另行取得 Token、Cookie、密碼，也不另行保存或向第三方傳送照片、位置或日期。
...
Chrome／Edge 的建議介面在 MAIN 環境存取網站 jQuery；Firefox 介面透過 `wrappedJSObject` 讀取原生建議資料。三個版本另在 MAIN 環境包裝網站既有的 `score_image`／`score_observation` 函式，只轉交原參數並讀取原回應，不具有擴充 API 權限，也不向頁面暴露帶擴充權限的回呼。Firefox manifest 保留 `websiteContent` 及 `personallyIdentifyingInfo` 的必要資料宣告。
