...
使用者主動套用、檢查、開啟自動模式或使用觀察頁鑑定建議時，插件沿用 iNaturalist 原有的 AI 建議請求。網站自行把辨識圖片及可用的位置／日期送到官方辨識服務，沿用網站登入驗證。插件在 MAIN 環境處理符合白名單的原生 fetch／XHR JSON 回應，只在頁面記憶體中取出分類 ID 與 `combined_score`。API v2 觀察請求若已限制回傳欄位卻省略 `combined_score`，插件只在同一請求的欄位投影補入這個布林欄位；不改照片、觀察、位置、日期、驗證或回應，不另行呼叫辨識 API、不另行取得 Token、Cookie、密碼，也不另行保存或向第三方傳送資料。
...
Chrome／Edge 的建議介面在 MAIN 環境存取網站 jQuery；Firefox 介面透過 `wrappedJSObject` 讀取原生建議資料。三個版本另在 MAIN 環境包裝頁面原有的 fetch／XHR，只對 iNaturalist HTTPS 主機下 `/v1`／`/v2` 的 CV 評分端點讀取 JSON，並只在上述 API v2 觀察欄位投影缺漏時補入 `combined_score`；相容舊頁面的 `window.inaturalistjs` 包裝不是必要資料路徑。這些 MAIN 腳本不具有擴充 API 權限，也不向頁面暴露帶擴充權限的回呼。Firefox manifest 保留 `websiteContent` 及 `personallyIdentifyingInfo` 的必要資料宣告。
