...
AI 預設要求第一項綜合分數嚴格大於 80；未達門檻或數字不可讀時，只有官方「非常確定它屬於這個……」提示出現，才填入該提示中的上階類群。也可選擇只填官方確定類群。綜合分數是 0–100 的排序分數，結合視覺與地點／日期，**不是實際正確率，也不應跨觀察比較**。插件只填草稿，最後由使用者檢查並上傳。詳見 [使用說明](docs/USAGE.md) 與 [資料及權限](docs/PRIVACY.md)。
