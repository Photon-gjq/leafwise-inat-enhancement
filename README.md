...
- **批次 AI 草稿**：預設保留已填的分類／手動文字，依分數套用第一個最佳建議，或填入官方確定的上階類群；支援預覽、自動處理新增空白照片、停止與明細。

AI 預設要求第一項視覺分數嚴格大於 80；未達門檻或數字不可讀時，只有官方「非常確定它屬於這個……」提示出現，才填入該提示中的上階類群。也可選擇只填官方確定類群。視覺分數是 0–100 的模型分數，**不是實際正確率**。插件只填草稿，最後由使用者檢查並上傳。詳見 [使用說明](docs/USAGE.md) 與 [資料及權限](docs/PRIVACY.md)。
