...
高階分類涵蓋分支排除、直接高階鑑定、Leaf taxa 歸併、多地點聯集、全部生物、全球範圍、重複請求合併、快取失效、逾時及錯誤輸入。上傳規則涵蓋 0／0.9／80／80.01／100、缺失分數、確定／不確定提示、首選順序與無效設定。0.11.2 新增官方確定類群回歸案例：當清單同時含「非常確定」的葉甲科，以及下方 5.39 分的黃帶芫菁屬時，官方模式必須選葉甲科；有提示文字卻讀不到可選的官方類群項目時必須保留原值。
