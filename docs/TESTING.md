...
高階分類涵蓋分支排除、直接高階鑑定、Leaf taxa 歸併、多地點聯集、全部生物、全球範圍、重複請求合併、快取失效、逾時及錯誤輸入。上傳規則涵蓋 0／0.9／80／80.01／100、缺失分數、確定／不確定提示、首選順序與無效設定；另驗證 `combined_score` 按 taxon ID 而非陣列順序配對、純視覺分數不作備援、緊湊標記沒有百分號、觀察頁共用樣式，以及個人次數強制刷新。分數模式下，首選嚴格高於門檻才選首選；等於或低於門檻時必須回退到可選的官方確定類群，兩者都不符合則保留原值。
