...
- **AI 建議增強**：上傳頁及每個具體觀察詳情頁的原生 AI 建議選單，以無框彩色數字顯示結合照片、地點與日期的綜合分數；上傳助手預設收合在「全選」旁，支援預覽、批次套用、自動處理、停止與明細。

AI 預設要求第一項綜合分數嚴格大於 80；未達門檻或數字不可讀時，只有官方「非常確定它屬於這個……」提示出現，才填入該提示中的上階類群。也可選擇只填官方確定類群。綜合分數是把網站原生回應的 0–1 `combined_score` 正規化後顯示的 0–100 排序分數，結合視覺與地點／日期；插件只被動讀取同一回應，不另發辨識請求。它**不是實際正確率，也不應跨觀察比較**。插件只填草稿，最後由使用者檢查並上傳。詳見 [使用說明](docs/USAGE.md) 與 [資料及權限](docs/PRIVACY.md)。
